...
- **Shared stack `shared/domain/`** — **fonte única de verdade** para os tipos e constantes de domínio (`Clef`, `Difficulty`, `InputMode`, `PitchData`, `IconName`, `InstrumentType`, `ManualType`, `INSTRUMENTS`, `MANUAL_TYPES`). **Não duplique** essas definições em outros arquivos — importe de `@/shared/domain`.
- **Anti-círculo** — `storage/` depende apenas de `theme/presets` e `theme/types` (dados), **nunca** de `theme/apply` (lógica). Se um default for necessário, ele vive em `theme/presets.ts`.
...
- **Zero `any`**: Todas as interfaces, callbacks, eventos e payloads devem ter tipos estritos. Tipos de domínio (Clef, PitchData, etc.) **devem** vir de `@/shared/domain` — nunca redeclarados localmente.
